...
if($usuario->checkCredenciales(array('Inscripciones'))) {  
    if($_REQUEST['ajax'] == 1) {
        ...
        $reconocidosbd = $reconocidos->getGuardadosByInscripcion($_REQUEST['inscripcionmatrimonio']); 
        $smarty->assign('reconocidosbd',$reconocidosbd); 
        $smarty->display('inscripciones/_listadoguardados.tpl');
    } else { 
        ...
        if(!$error) {
            header("Location: /modulos/inscripciones/editdisolucionmatrimonio.php?id=".$_POST['idinscripcion']);    
        }
   }        
} else {
