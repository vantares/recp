...
if($usuario->checkCredenciales(array('Inscripciones'))) { 
	...
	if((isset($_POST['salvar'])) && ($_POST['salvar'] == 'salvar')) {
		...
		if(!$error) { 
			...
			$_REQUEST['tipoinscripcion'] =  'Matrimonios';
			...
			if(!$error) { 
				...
				if(!$error) {
					...
					if(!$error) {             
						...
						$_REQUEST['idmatrimonio'] = $actojuridico->getlastId();
		...
		if(!$error) {
			//Registro el evento
			$evento = new evento();
			$evento->request['tipoevento'] = 'Crear Partida de Matrimonio';
			...
			$evento->request['descripcion'] = 'Se creo una partida de matrimonio con tomo '.$_REQUEST['idtomo'].' folio '.$_REQUEST['folio'].' Se hizo el asiento correspondiente'; 
	...
	$smarty->assign('url','/modulos/inscripciones/addmatrimonio.php'); 
	...
	$smarty->assign('camino','>> Inscripciones >> Matrimonios >> Nueva'); 
