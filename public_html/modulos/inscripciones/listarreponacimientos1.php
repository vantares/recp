...
if($usuario->checkCredenciales(array('Inscripciones'))) { 
    ...
    if($filter == ''){    
        $inscripcionesbd = $inscripcion->readDataFilter("inscripcion.tipoinscripcion='Reposicion Nacimiento' ");
        ...
        $inscripcionesbd = $inscripcion->readDataSQL("SELECT inscripcion.*,
                                                  ...
                                                  INNER JOIN tomo
                                                       ...
                                                       WHERE inscripcion.tipoinscripcion='Reposicion Nacimiento' ".
                                                             $filter);
    }   
    $cadena = new TCadena();  
    ...
    $smarty->assign('pagina','editreponacimiento.php');   
    $smarty->assign('fechainicial',$_REQUEST['fechainicial']);
    $smarty->assign('fechafinal',$_REQUEST['fechafinal']); 
    $smarty->assign('inscripcion','reponacimiento');  
    $smarty->assign('arrayInscripcion',$cadena->convert_sqlData_toString($inscripcionesbd));
    $smarty->assign('urlnueva','/modulos/inscripciones/addreponacimiento.php');
    $smarty->assign('template','inscripciones/listadoinscripciones.tpl'); 
    $smarty->assign('titular','Listado de Repociciones de Nacimiento'); 
    $smarty->assign('camino','Inscripcion >> Reposicion Nacimiento >> Listado'); 
