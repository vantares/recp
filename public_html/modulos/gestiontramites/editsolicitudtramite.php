	...
	switch($action){
		...
		case 'add':
			...
			if($excento){
				...
				// echo "creando recibo";
				$recibo= new recibo();
				if(isset($_REQUEST['idrecibo']) && !empty($_REQUEST['idrecibo'])){
					$recibobd = $recibo->getRecibo($_REQUEST['idrecibo']); 
					$_SESSION['idrecibo']= $recibobd->request['idrecibo'];
				}
				else{
					$_REQUEST['nombrecliente'] = $_REQUEST['nombre'];
					$_REQUEST['monto'] = 0;
		...
		case 'save':
		case 'update':
			//salvar datos en caso de modificacion
			$solicitud=new solicitudtramite();
			$solicitud->readEnv();
			$solicitud->request['estado']= 'PENDIENTE';
			...
			switch ($tiposolicitud){
				case 1: 
					...
					// set attribute value
					$solicitudtramite->readEnv();
					$solicitudtramite->request['idsolicitudinscripcion'] = $solicitud->request['idsolicitudtramite'];  
				...
				case 2: 
					...
					// set attribute value
					$solicitudtramite->readEnv();
					$solicitudtramite->request['idsolicitudcertificacion'] = $solicitud->request['idsolicitudtramite'];  
					$solicitudtramite->updateRecord();
					// echo "ingresando solicitud certificacion";
					break;
			}
			...
			if($excento){
				$patrocinio = new patrocinio();
				$patrocinio->request['idsolicitudtramite'] = $solicitud->request['idsolicitudtramite'];
				$patrocinio->request['idorganizacion'] = $_REQUEST['idorganizacion'];
				$patrocinio->updateRecord();
			}
			else{
				// echo "creando recibo";
				$recibo= new recibo();
				...
				$pago = new pagoTable();    
				$_REQUEST['idsolicitudtramite'] =  $solicitud->request['idsolicitudtramite'];
				$pago->readEnv();
				$pago->updateRecord(); 
			}
			// obtener datos mostrar en el formulario
			if(isset($_GET['idsolicitud'])){
				$solicitud=new solicitudtramite();
				$solicitud->readEnv();
				$solicitud->getSolicitudtramite($_REQUEST['idsolicitud']);
