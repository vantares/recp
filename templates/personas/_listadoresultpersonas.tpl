...
$(document).ready(function() {
    $("#listado" + $("#tipo").val()).click(function(evento){
        if(evento.target.id != '') {
            ...
            $("#" + $("#tipo").val()).load("/modulos/personas/__cargarpersona.php", {defuncionb: $("#defuncionb").val(), tipo: $("#tipo").val(), idpersona: $("#idpersona_" + nombre[1]).val(), template: $("#module").val() + '/_' + $("#tipo").val() + '.tpl'},
            function(){
...
{/literal} 
{if $arrayresultados}
    <input  type="hidden"  name="defuncionb" id="defuncionb" value="{$defuncionb}"/>
